        ...
        >
            {openFiles.map((file) => (
                <span
                    key={file.id}
                    className={cn(
                        "flex w-fit cursor-pointer items-center rounded-t-md px-2 py-1 text-white transition-all duration-200",
                        { "bg-darkHover file-tab-active pulse glow-float": file.id === activeFile?.id }
                    )}
