...
function RunView() {
    ...
    const runHtmlInBrowser = () => {
        // Only allow if selected language is HTML or file extension is .html
        if (
            selectedLanguage.language.toLowerCase().includes("html") ||
            (activeFile?.name?.toLowerCase().endsWith(".html"))
        ) {
            const htmlContent = output || activeFile?.content || ""
            const blob = new Blob([htmlContent], { type: "text/html" })
            const url = URL.createObjectURL(blob)
            window.open(url, "_blank", "noopener,noreferrer")
        } else {
            toast.error("Open or select an HTML file to run in browser")
        }
    }
        ...
        >
            ...
            <div className="flex h-[90%] w-full flex-col items-end gap-2 md:h-[92%]">
                ...
                {(selectedLanguage.language.toLowerCase().includes("html") ||
                    (activeFile?.name?.toLowerCase().endsWith(".html"))) && (
                    <button
                        className="flex w-full justify-center rounded-md bg-primary p-2 font-bold text-black outline-none mt-1"
                        onClick={runHtmlInBrowser}
                        type="button"
                    >
                        Run HTML in Browser
                    </button>
